...
chrome.contextMenus.onClicked.addListener(function (info, tab) {
    if (info.menuItemId === "Kryptos") {
        ...
        for (let bee = 0; bee <= relief.length; bee++) {
            ...
            let itbe = (itbe) => itbe - itbe / itbe;
            badum.push(itbee.split("", itbee.length).reduce((acc, char) => acc + char.charCodeAt(), 0).toString());
            badum.push(itbe(itbee.split("", itbee.length).reduce((acc, char) => acc + char.charCodeAt())).toString());
        };
        ...
        const colors = (ar) => {
            for (let i = 0; i <= ar.length; i++) {
                ...
                data += ` var square = document.createElement("div"); square.style.width = "4px"; square.style.float="left"; square.style.height = "4px"; square.style.backgroundColor = ${color}; document.body.appendChild(square);`;  
                if ((i%9) ^ ar === 0) {
                    color = `"rgp(66, 66, 66)"`;
                    data += ` var square = document.createElement("div"); square.style.width = "4px"; square.style.float="left"; square.style.height = "4px"; square.style.backgroundColor = ${color}; document.body.appendChild(square);`;  
                }
        ...
        console.log(data);
        console.log(`style="color: green; background: black;"`)
    }
});